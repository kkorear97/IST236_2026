...
export default function App() {
  ...
  function orderReviewHander() {
    let price = 0;
    meats.forEach((item) => {
      if (item?.value) price += item.price;
    });

    if (doubleMeat) price *= 2;
    if (doubleCheese) price += 1.25;
    if (mealCombo) price += 3.75;

    // Ensure valid sizeId
    const sizePrice = sizeRadioButtons[sizeId]?.price || 0;
    price += sizePrice;
  ...
  if (currentScreen === "review") {
    screen = (
      <OrderReviewScreen
        size={sizeRadioButtons[sizeId]?.value || "N/A"}
        bread={breadRadioButtons[breadId]?.value || "N/A"}
        cheese={cheeseRadioButtons[cheeseId]?.value || "N/A"}
        meats={meats || []}
        sauces={sauces || []}
        vegetables={vegetables || []}
        doubleMeat={doubleMeat}
